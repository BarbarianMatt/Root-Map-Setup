...
export class RootlogService {
  ...
  public getPaths(map: RootMap): Record<string, [number, number]> {
    return pathPositions[map] || {};
  }

  public getPathPosition(map: RootMap, forestKey: string): [number, number] {
    return pathPositions[map][forestKey] || [0, 0];
  }
  public displayRelics(state: RootGameState, faction: Record<string,string> | any): void {
    if (faction.key =='K')
  ...
  public lootRuin(building: any, clearing: any) {
    ...
    clearing.buildings['m_b_m']=clearing.buildings['m_b_w'];
  }
  public getFactionName(faction: RootFaction| string): string {
    return factionNames[faction as RootFaction];
  }
  public getFactionNameAndDeck(faction: Record<string,string> | any): string {
    //if (typeof faction == Record<string,string>)
    if (faction.key=='M'){
      if (faction.value == 'E&P Deck')
        return 'e&p'
      return 'standard'
    }
    return factionNames[faction.key as RootFaction];
  }
        ...
        switch (pieceType) {
          case RootPieceType.Building: {
            ...
            if (destinationForest) {// && curState.forests[destinationForest]) {
              var forest=destinationForest.split('_');
              if (forest[0]==forest[1])
                curState.forests[destinationForest].tokens[formattedPiece] = num;
              else
                curState.forests[destinationForest].tokens[formattedPiece] = num;
            }
            break;
